...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Plot Logistic Regression results (saved outputs).")
    p.add_argument("--out_dir", default="../outputs/logreg_top30", help="Same out_dir used by run_logreg.py")
    p.add_argument("--top_k", type=int, default=10, help="Top-K items for error bars / class plots")
    p.add_argument("--confusion_max_classes", type=int, default=30, help="Max classes to include in confusion heatmap")
...
def plot_metrics_bar(metrics: Dict[str, float], out_path: str, dpi: int) -> None:
    ...
    plt.ylim(0.0, 1.0)
    plt.ylabel("Score")
    plt.title("Logistic Regression – Overall Metrics")
    plt.tight_layout()
...
def plot_top_offdiag_bar(spark: SparkSession, decoded_path: str, out_path: str, top_k: int, dpi: int) -> None:
    ...
    plt.xlabel("Count")
    plt.title(f"Top {len(labels)} Off-Diagonal Confusions")
...
def plot_confusion_heatmap(
    ...
    plt.title(f"Confusion Matrix (Top {len(labels_sorted)} Labels by Frequency)")
...
def main() -> None:
    ...
    spark = SparkSession.builder.appName("plot-logreg-results").getOrCreate()
