...
def load_label_list_from_feature_pipeline(feature_pipeline_dir: str) -> List[str]:
    ...
    if sim is None:
        raise RuntimeError(
            "StringIndexerModel (outputCol='label') not found in feature pipeline model."
        )
...
def decode_confusions(conf_df, labels: List[str]):
    ...
    decoded = (
        conf_df.join(
            mapping.withColumnRenamed("idx", "label").withColumnRenamed(
                "name", "label_name"
            ),
            on="label",
            how="left",
        ...
        .join(
            mapping.withColumnRenamed("idx", "prediction").withColumnRenamed(
                "name", "pred_name"
            ),
            on="prediction",
            how="left",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Train/evaluate multinomial Logistic Regression on TF-IDF features."
    )
    p.add_argument(
        "--tfidf_parquet",
        default="../outputs/tfidf_top30/dataset.parquet",
        help="Parquet path containing columns: id, label_str, label, features, update_date",
    )
    p.add_argument(
        "--feature_pipeline_dir",
        default="../outputs/tfidf_top30/feature_pipeline_model",
        help="Saved PipelineModel dir used to extract label order from StringIndexerModel.",
    )
    p.add_argument(
        "--out_dir",
        default="../outputs/logreg_top30",
        help="Output directory for model + metrics + confusions.",
    )
    p.add_argument("--max_iter", type=int, default=50, help="LR maxIter")
    p.add_argument("--reg_param", type=float, default=0.1, help="LR regParam")
    p.add_argument(
        "--elastic_net",
        type=float,
        default=0.0,
        help="LR elasticNetParam (0=ridge, 1=lasso).",
    )
    p.add_argument(
        "--train_frac",
        type=float,
        default=0.8,
        help="Train split fraction (rest is test).",
    )
    p.add_argument(
        "--repartition",
        type=int,
        default=16,
        help="Number of partitions for the training DataFrame.",
    )
    p.add_argument(
        "--driver_memory",
        default="6g",
...
def main() -> None:
    ...
    spark = make_spark("arxiv-logreg", driver_memory=args.driver_memory)

    df = (
        spark.read.parquet(args.tfidf_parquet)
        .select("id", "label_str", "label", "features", "update_date")
    )
