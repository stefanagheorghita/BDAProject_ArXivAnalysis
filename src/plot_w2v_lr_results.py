...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Plot Word2Vec+LR results (saved outputs).")
    p.add_argument("--out_dir", default="../outputs/w2v_lr_top30_full")
    p.add_argument("--top_k", type=int, default=10)
    p.add_argument("--confusion_max_classes", type=int, default=30)
...
def plot_metrics_bar(metrics: Dict[str, float], out_path: str, dpi: int) -> None:
    ...
    plt.ylim(0.0, 1.0)
    plt.ylabel("Score")
    plt.title("Word2Vec + Logistic Regression – Overall Metrics")
    plt.tight_layout()
    plt.savefig(out_path, dpi=dpi)
    plt.close()


def plot_top_offdiag_bar_raw(
        spark: SparkSession,
        raw_path: str,
        out_path: str,
        top_k: int,
    ...
    df = spark.read.parquet(raw_path).orderBy("count", ascending=False).limit(top_k)
    pdf = df.toPandas()
    if pdf.empty:
        raise RuntimeError("No rows found in off-diagonal confusions parquet.")

    labels = [f"{int(l)} → {int(p)}" for l, p in zip(pdf["label"], pdf["prediction"])]
    counts = pdf["count"].astype(int).tolist()

    plt.figure(figsize=(10, max(3.5, 0.4 * len(labels))))
    plt.barh(labels, counts)
    plt.xlabel("Count")
    plt.title(f"Top {len(labels)} Off-Diagonal Confusions (raw label ids)")
...
def plot_confusion_heatmap(
    ...
    plt.title(f"Confusion Matrix (Top {len(labels_sorted)} Labels) – W2V+LR")
...
def main() -> None:
    ...
    offdiag_raw_path = os.path.join(out_dir, "top_confusions_offdiag_raw.parquet")

    metrics = read_metrics_txt(metrics_path)
    plot_metrics_bar(metrics, os.path.join(fig_dir, "metrics_bar.png"), dpi=args.dpi)

    spark = SparkSession.builder.appName("plot-w2v-lr-results").getOrCreate()

    plot_top_offdiag_bar_raw(
        spark,
        raw_path=offdiag_raw_path,
