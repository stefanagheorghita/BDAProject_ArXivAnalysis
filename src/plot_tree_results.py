...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Plot DecisionTree results (saved outputs).")
    p.add_argument("--out_dir", default="../outputs/tree_top30", help="Same out_dir used by run_tree.py")
...
def plot_metrics_bar(metrics: Dict[str, float], out_path: str, dpi: int) -> None:
    ...
    plt.ylim(0.0, 1.0)
    plt.ylabel("Score")
    plt.title("Decision Tree – Overall Metrics")
    plt.tight_layout()
...
def plot_top_offdiag_bar(spark: SparkSession, decoded_path: str, out_path: str, top_k: int, dpi: int) -> None:
    ...
    plt.xlabel("Count")
    plt.title(f"Top {len(labels)} Off-Diagonal Confusions (DT)")
...
def plot_confusion_heatmap(
    ...
    plt.title(f"Confusion Matrix (Top {len(labels_sorted)} Labels by Frequency) – DT")
...
def main() -> None:
    ...
    spark = SparkSession.builder.appName("plot-tree-results").getOrCreate()
