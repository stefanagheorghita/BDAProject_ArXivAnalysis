...
from modeling.train_tree import build_tree_pipeline
...
def decode_confusions(conf_df, labels: List[str]):
    ...
    mapping = spark.createDataFrame([(float(i), lab) for i, lab in enumerate(labels)], ["idx", "name"])
    decoded = (
        conf_df.join(
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train/evaluate DecisionTree on TF-IDF features.")
    ...
    p.add_argument("--out_dir", default="../outputs/tree_top30")

    p.add_argument("--driver_memory", default="12g")
    p.add_argument("--shuffle_partitions", type=int, default=64)

    p.add_argument("--train_frac", type=float, default=0.8)
    p.add_argument("--train_sample", type=float, default=0.10)
    p.add_argument("--test_sample", type=float, default=0.25)
    p.add_argument("--repartition", type=int, default=4)

    p.add_argument("--max_depth", type=int, default=6)
    p.add_argument("--max_bins", type=int, default=16)
    p.add_argument("--min_instances_per_node", type=int, default=20)
...
def main() -> None:
    ...
    spark = make_spark("arxiv-tree-top30", driver_memory=args.driver_memory)
    ...
    print("Saved DT model to:", model_dir)
