...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Plot Naive Bayes results (saved outputs).")
    p.add_argument("--out_dir", default="../outputs/nb_top30", help="Same out_dir used by run_nb.py")
    p.add_argument("--top_k", type=int, default=10, help="Top-K off-diagonal confusions to show")
...
def plot_metrics_bar(metrics: Dict[str, float], out_path: str, dpi: int) -> None:
    ...
    plt.ylim(0.0, 1.0)
    plt.ylabel("Score")
    plt.title("Naive Bayes – Overall Metrics")
    plt.tight_layout()
    plt.savefig(out_path, dpi=dpi)
    plt.close()


def plot_top_offdiag_bar(
    spark: SparkSession,
    decoded_path: str,
    out_path: str,
    top_k: int,
    ...
    plt.xlabel("Count")
    plt.title(f"Top {len(labels)} Off-Diagonal Confusions (NB)")
...
def plot_confusion_heatmap(
    ...
    plt.title(f"Confusion Matrix (Top {len(labels_sorted)} Labels by Frequency) – NB")
...
def main() -> None:
    ...
    spark = SparkSession.builder.appName("plot-nb-results").getOrCreate()
