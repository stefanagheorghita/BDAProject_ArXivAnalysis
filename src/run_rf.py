...
from modeling.train_rf import build_rf_pipeline
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train/evaluate RandomForest on TF-IDF features.")
    ...
    p.add_argument("--out_dir", default="../outputs/rf_top30")

    p.add_argument("--driver_memory", default="12g")
    p.add_argument("--shuffle_partitions", type=int, default=64)

    p.add_argument("--train_frac", type=float, default=0.8)
    p.add_argument("--train_sample", type=float, default=0.10, help="Fraction of training data to fit RF on.")
    p.add_argument("--test_sample", type=float, default=0.25, help="Fraction of test data to evaluate on.")
    p.add_argument("--repartition", type=int, default=4)

    p.add_argument("--num_trees", type=int, default=200)
    p.add_argument("--max_depth", type=int, default=8)
    p.add_argument("--max_bins", type=int, default=32)
    p.add_argument("--min_instances_per_node", type=int, default=10)
    p.add_argument("--feature_subset_strategy", default="sqrt")
...
def main() -> None:
    ...
    spark = make_spark("arxiv-rf-top30", driver_memory=args.driver_memory)
    ...
    train_df = train_df.sample(withReplacement=False, fraction=args.train_sample, seed=SEED).repartition(args.repartition)
    test_df = test_df.sample(withReplacement=False, fraction=args.test_sample, seed=SEED).repartition(args.repartition)
    ...
    pipe = build_rf_pipeline(
        ...
        min_instances_per_node=args.min_instances_per_node,
        feature_subset_strategy=args.feature_subset_strategy,
        seed=SEED,
    )
    ...
    print("Saved RF model to:", model_dir)
